...
# common parser for TOO detection
parent_too = argparse.ArgumentParser(add_help = False)

# testing matrix (TPM)
parent_too.add_argument(
    '-tst', '--testing_exp',
    required = True
)
...
parser_conversion.add_argument(
    ...
    help = ('Path of gene expression file (.tsv).\n' + 
            ...
            'Expression values must be FPKM or RPKM.'
         )
)
...
parser_f_ratio.add_argument(
    '-s', '--single_cell',
    default = './required_data/f_ratio/CTC-pooled_tpm.tsv',
    help = ''
)
...
parser_f_ratio.add_argument(
    '-b', '--bulk',
    default = './required_data/f_ratio/TCGA-pooled_tpm.tsv',
    help = ''
)
